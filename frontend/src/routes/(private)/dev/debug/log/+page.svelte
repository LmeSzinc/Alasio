<script lang="ts">
  ...
  const sampleLogs: LogDataProps[] = [
    ...
    { t: now - 15, l: "INFO", m: "Task [Main] complete — elapsed: 5m 12s" },
    {
      t: now - 12,
      l: "ERROR",
      m: "Screenshot capture failed: ADB connection reset by peer",
    ...
    { t: now - 8, l: "INFO", m: "ADB reconnected — session restored" },
    {
      t: now - 6,
      l: "CRITICAL",
      m: "Config validation failed: scheduler.cron expression invalid",
    ...
    { t: now - 4, l: "WARNING", m: "Falling back to default scheduler config" },
    {
      t: now - 2,
      l: "CRITICAL",
      m: "Out of memory: Python heap 2.1 GiB / 2.0 GiB limit",
    ...
    // Long info log to test auto line break
    {
      t: now,
      l: "INFO",
      m: "Task [ResourceScan] completed — scanned 1,247 asset files across 38 directories (buttons: 412, templates: 689, ui_defs: 146), matched 1,203 against known hashes, queued 44 new assets for optimization pipeline, cache hit ratio 91.7%, elapsed: 2.34s",
    },
    // Raw log (single long line, no timestamp prefix) to test horizontal overflow
    {
      t: now,
      l: "RAW",
      m: `[2026-06-05 10:15:23] [adbutils]  D  AdbDevice(serial="127.0.0.1:5555", transport_id=42, state="device")  |  serial=127.0.0.1:5555  |  properties=[ro.product.model=Pixel_7_Pro, ro.build.version.sdk=34, ro.build.version.release=14]  |  features=[shell_v2, cmd, stat_v2, list_v2, fixed_push_symlink, apex, abb, abb_exec, send_recv_v2, send_recv_v2_burst, remount_shell, swa]  |  screen=1080x2340@420  |  density=420  |  rotation=0`,
      r: 1,
    },
  ];
</script>
...
<div class="mx-auto flex h-full w-full flex-col gap-6 overflow-auto p-4">
  ...
  <section class="flex flex-col gap-2">
    ...
    <p class="text-muted-foreground/70 text-xs">Log area constrained to 300px — scroll to reveal older entries.</p>
