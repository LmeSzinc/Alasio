...
"""
    ...
    def test_install_entry_points_warning(self, fs, pyc_compile):
        ...
        assert [(log['l'], log['m']) for log in capture.backend.logs] == [
            ('WARNING', 'console_scripts of the wheel are not generated by SimplePip'),
            ('INFO', f'Installing demo to {site}'),
            ('INFO', 'Compiling 1 py files'),
            ('INFO', 'Successfully installed demo'),
        ]
        assert capture.fd.any_contains('console_scripts of the wheel are not generated by SimplePip')
    ...
    def test_uninstall(self, fs, pyc_compile):
        """Uninstalling removes every installed file, the summary of it is logged."""
        site = site_packages(fs)
        wheel = build_wheel('/w/demo-1.0-py3-none-any.whl', {
            'demo/__init__.py': b'a = 1\n',
            'demo-1.0.data/data/share/demo/data.txt': b'demo data\n',
        ...
        assert [(log['l'], log['m']) for log in capture.backend.logs] == [
            ('INFO', 'Uninstalled demo: 8 files, 5 folders removed'),
        ]
        assert list_files(abs_path(fs, '/env')) == []
        assert list_folders(site) == []

    def test_uninstall_missing_package(self, fs):
        """Uninstalling a package that is not installed logs nothing."""
        site = site_packages(fs)
        with logger.mock_capture_writer() as capture:
